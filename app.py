...
# -----------------------------
# BCG-style matrix using Altair
# -----------------------------
st.subheader("📊 BCG-style Account Matrix (Altair)")

bcg_chart = alt.Chart(filtered_df).mark_circle(size=100).encode(
    x=alt.X("growth_score", title="Growth Score"),
    y=alt.Y("priority_score", title="Priority Score"),
    color=alt.Color("segment:N"),
    tooltip=["account_name","priority_score","risk_score","growth_score","attention_score"]
).interactive()

st.altair_chart(bcg_chart, use_container_width=True)
...
if not account_row.empty:
    ...
    with st.expander("View reasoning and supporting metrics"):
        metrics_list = [
            ...
            "latest_nps","expansion_pipeline_gbp","seats_used","seats_purchased",
            "open_leads_count","arr_gbp","latest_note_date"
        ]
