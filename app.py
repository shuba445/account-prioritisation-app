...
def get_column(df, col_name):
    if col_name in df.columns:
        return df[col_name]
    matches = difflib.get_close_matches(col_name, df.columns, n=1, cutoff=0.6)
    if matches:
        st.warning(f"Column '{col_name}' not found. Using '{matches[0]}' instead.")
        return df[matches[0]]
    st.warning(f"Column '{col_name}' not found. Using default 0.")
...
def compute_scores(df):
    ...
    # Priority
    df["priority_score"] = df.get("risk_score",0)*0.5 + df.get("growth_score",0)*0.5 + df.get("attention_score",0)*0.2
